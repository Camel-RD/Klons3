...
namespace KlonsF.Forms
{
    public partial class Form_LOPS : MyFormBaseF
    {
            ...
            int startindex = 0, bool forward = true)
        {
            ...
            var rv = bsLOPS[startindex] as ObjectView<F_LOPS>;
            if (rv == null) return -1;
            ...
            {
                var rv1 = bsLOPS[i] as ObjectView<F_LOPS>;
        ...
        private void tsbDocLog_Click(object sender, EventArgs e)
        {
            ...
            var row = (bsLOPS.Current as ObjectView<F_LOPS>)?.Object;
            if (row == null) return;
            var frm = MyMainForm.ShowForm(typeof(Form_LogDoc)) as Form_LogDoc;
            frm.GetData2(row.DOCID.Value);
        }
    }
}
