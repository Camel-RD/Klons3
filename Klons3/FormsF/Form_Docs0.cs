...
namespace KlonsF.Forms
{
    public partial class Form_Docs0 : MyFormBaseF
    {
        ...
        {
            var scw = MyData.Settings.ColumnWidths_Docs2;
            (int ver, int[] cw) = dgvDocs0.ParseColumnWidths(scw);
            ...
            (ver, cw) = dgvDocs0.ParseColumnWidths(scw);
            if (cw.Length > 0)
            {
                dgcAC.Width = CW(cw[2]);
            }
            ...
            int startindex = 0, bool forward = true)
        {
            ...
            var rv = bsDocs0[startindex] as ObjectView<ModelsF.F_DOCS0>;
            if (rv == null) return -1;
            var propdescr = TypeDescriptor.GetProperties(rv).Find(propname, true);
            ...
            {
                var rv1 = bsDocs0[i] as ObjectView<ModelsF.F_DOCS0>;
                o = propdescr.GetValue(rv1);
