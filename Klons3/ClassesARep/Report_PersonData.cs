...
namespace KlonsA.Classes
{
    public class Report_PersonData
    {
        ...
        public void ShowReport(A_PERSONS dr_person, DateTime dt)
        {
            var table_persons = MyData.DbContextA.BL_A_PERSONS;
            var table_persons_r = MyData.DbContextA.BL_A_PERSONS_R;
            var table_positions = MyData.DbContextA.BL_A_POSITIONS;
            var table_positions_r = MyData.DbContextA.BL_A_POSITIONS_R;

            var drs_person = new[] { dr_person };
            var drs_persons_r = dr_person.PersonsStateRows
                .OrderBy(d => d.EDIT_DATE)
                .ToArray();

            var drs_positions = dr_person.Positions;
            ...
            rrs_positions = new RepRowPosition[drs_positions.Count];

            for (int i = 0; i < drs_positions.Count; i++)
            {
                var dr_positions_r = drs_positions[i].PositionStateRows
    ...
    public class RepRowPerson
    {
        ...
        public void SetFromPersonsDataRow(A_PERSONS dr)
        {
            ID = dr.ID;
            FNAME = dr.FNAME;
            LNAME = dr.LNAME;
            ...
            PASSPORT_DATE = dr.PASSPORT_DATE;
            PHONE = dr.PHONE;
            EMAIL = dr.EMAIL;
            BANK_ID = dr.BANK_ID;
        ...
        public void SetFromPersonsDataRow(A_PERSONS_R dr)
        {
            ID = dr.ID;
            IDP = dr.IDP;
        ...
        {
            var dr = KlonsData.St.DbContextA.A_BANKS.Find(id);
            if (dr == null) return;
            BANK_SID = dr.SID;
            BANK_NAME = dr.NAME;
        }
        ...
        {
            var dr_persons_r = DataTasksA.GetPersonsR(IDP, dt);
            var dr_rates = DataTasksA.GetRates(dt);
        ...
        public RepRowPersonStatusChange[] GetStatusChangeRep()
        {
            ...
            for(int i = 1; i < drs_persons_r.Length; i++)
            {
                ...
                {
                    if (s != "") s += "\n";
                    s += $"apgādājamo skaita izmaiņa no {dr_prev.APGAD_SK} uz {dr_cur.APGAD_SK}";
                ...
                if (dr_cur.INVALID != dr_prev.INVALID)
                {
                    ...
                    if(dr_cur.INVALID == 0)
                        ...
                        s += $"piešķirta invaliditātes {dr_cur.INVALID}. grupa";
                }
        ...
        public List<KeyValuePair<DateTime, CalcRRow2>> GetDatesAndRates(DateTime dte)
        {
            var table_persons = MyData.DbContextA.BL_A_PERSONS;
            var table_persons_r = MyData.DbContextA.BL_A_PERSONS_R;
            var table_events = MyData.DbContextA.BL_A_EVENTS;
            var table_rates = MyData.DbContextA.BL_A_RATES;
            var table_untaxedmin = MyData.DbContextA.BL_A_UNTAXED_MIN;
    ...
    public class RepRowPosition
    {
        ...
        public void SetFromPositionDataRow(A_POSITIONS dr)
        {
            ID = dr.ID;
            IDP = dr.IDP;
            IDDEP = dr.IDDEP;
        ...
        public void SetFromPositionDataRow(A_POSITIONS_R dr)
        {
            IDR = dr.ID;
            IDDEP = dr.IDDEP;
            if (IDDEP != null)
                DEP = MyData.DbContextA.A_DEPARTMENTS.Find(IDDEP)?.DESCR;
            TITLE = dr.TITLE;
            ...
            if (OCCUPATION_CODE != null)
                OCCUPATION_CODE_S = MyData.DbContextA.A_PROFESSIONS.Find(OCCUPATION_CODE)?.DESCR;
            USED = dr.USED;
            EDIT_DATE = dr.EDIT_DATE;
        ...
        public void GetRatesHistory()
        {
            ...
            var dr_pos = MyData.DbContextA.A_POSITIONS.Find(ID);
            if (dr_pos == null) return;
            var drs_pos_r = dr_pos.PositionStateRows
                .OrderBy(d => d.EDIT_DATE)
                .ToArray();
    ...
    public class RepRowEvent
    {
        ...
        public void SetFromDataRow(A_EVENTS dr)
        {
            ID = dr.ID;
            PersonRow = dr.Person;
            PositionRow = dr.Position;
            IDN = dr.IDN;
            IDN2 = dr.IDN2;
            DATE1 = dr.DATE1;
            DATE2 = dr.DATE2;
            ...
            if (PositionRow != null)
                PosTitle = PositionRow.TITLE;
            EventName = MyData.DbContextA.A_EVENT_TYPES.Find(IDN)?.DESCR;
            if (IDN2 != null)
                Event2Name = MyData.DbContextA.A_EVENT_TYPES.Find(IDN2.Value)?.DESCR;
        }
