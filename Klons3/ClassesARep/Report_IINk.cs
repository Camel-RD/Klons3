...
namespace KlonsA.Classes
{
    public class Report_IINk
    {
        ...
        public List<IINReportRow> MakeReport1(EReportType reporttype, DateTime dt1, DateTime dt2, DateTime dtx, bool simple)
        {
            ...
            List<SP_A_REP_IINK_01> table = null;
            if (MyData.Params.IINSimple)
            {
                if (reporttype == EReportType.ForMonth)
                {
                    table = DataLoaderA.GetBy_SP_A_REP_IINK_02(dt1, dt2, new DateTime(dt1.Year, 1, 1));
                }
                else if (reporttype == EReportType.ForYear)
                {
                    table = DataLoaderA.GetBy_SP_A_REP_IINK_03(dt1, dt2);
                }
            }
            else
            {
                if (reporttype == EReportType.ForMonth)
                {
                    table = DataLoaderA.GetBy_SP_A_REP_IINK_32(dt1, dt2, dtx);
                }
                else if (reporttype == EReportType.ForYear)
                {
                    table = DataLoaderA.GetBy_SP_A_REP_IINK_33(dt1, dt2, dtx);
                }
            }
            ...
            for (int i = 0; i < table.Count; i++)
            {
                ...
                var drp = MyData.DbContextA.BL_A_PERSONS.FindById(rr.idp);
        ...
        public List<IINReportRow> MakeReport2(EReportType reporttype, DateTime dt1, DateTime dt2)
        {
            ...
            List<SP_A_REP_IINK_21> table = null;
            if (reporttype == EReportType.ForMonth)
            {
                table = DataLoaderA.GetBy_SP_A_REP_IINK_21(dt1.Year, dt1.Month, dt1, dt2);
            }
            else if (reporttype == EReportType.ForYear)
            {
                table = DataLoaderA.GetBy_SP_A_REP_IINK_22(dt1.Year, dt1, dt2);
            }
            ...
            for (int i = 0; i < table.Count; i++)
            {
                ...
                var drp = MyData.DbContextA.A_PERSONS_FIZ.Find(rr.idp);
                rr.PK = PKForRep(drp.PK);
                rr.Name = drp.FNAME + " " + drp.LNAME;
                ...
                rr.IncomeTypeS = MyData.DbContextA.A_INCOME_CODES.Find(rr.IncomeType)?.DESCR;
        ...
        {
            var dr = MyData.DbContextA.A_INCOME_CODES.Find(id);
            return dr?.DESCR;
        }
    }
    ...
    public class IINReportRow : INotifyPropertyChanged
    {
        ...
        public string PeriodStr => $"{Date1:dd.MM.yyyy} - {Date2:dd.MM.yyyy}";
