...
{
    public partial class FormP_TaxDeprecYR : MyFormBaseF
    {
        public FormP_TaxDeprecYR()
        {
            InitializeComponent();
        ...
        {
            SetSaveButton(MyData.DbContextP.HasChanges());
        }

        private void SetSaveButton(bool red)
        {
            tsbSave.Image = red ?
                KlonsF.Properties.Resources.Save2 : 
                KlonsF.Properties.Resources.Save1;
        }

        public bool GetYears(out int yr1, out int yr2)
        {
            yr1 = yr2 = 0;
            var drs = MyData.DbContextP.P_TAXDEPRECYEAR.Local.ToList();
        ...
        public void SetCurrentYear(int yr)
        {
            ...
            YearCurrent = yr;
            if (yr == 0) return;
            bsRows.SetFilter<P_TAXDEPRECYEAR>(x => x.YR == yr);
        ...
        {
            int yr = 0;
            if (MyData.DbContextP.P_ITEMS.Local.Count == 0)
            {
            ...
            if (string.IsNullOrEmpty(fm.SelectedValueStr))
            {
                MyMainForm.ShowWarning("Netika norādīts gads.");
                return 0;
            }
            if (!int.TryParse(fm.SelectedValueStr, out yr) || yr < 1900 || yr > 2100)
            {
                MyMainForm.ShowWarning("Norādīts nekorekts gads.");
                return 0;
            }

            if (MyData.DbContextP.P_TAXDEPRECYEAR.Where(d => d.YR == yr).Any())
            {
        ...
        public List<TaxDeprecCalcInfo> CalcYear(int yr, bool addstartvalues)
        {
            ...
            var table_deprec = MyData.DbContextP.P_TAXDEPRECYEAR.Local.ToList();
            var table_catt = MyData.DbContextP.P_CATT.Local.ToList();

            var drs_new = new List<TaxDeprecCalcInfo>();
            var drs_prev = table_deprec.Where(d => d.YR == yr - 1).ToList();

            foreach (var evr in ReportMovement.ReportRows)
            {
                var dr_new = new TaxDeprecCalcInfo();
                var dr_catt = MyData.DbContextP.P_CATT.Find(evr.CatT);
            ...
            {
                var dr_new = new TaxDeprecCalcInfo();
                var dr_catt = MyData.DbContextP.P_CATT.Find(dr_prev.ID);
                drs_new.Add(dr_new);
        ...
        public bool UpdateYear(int yr, bool addstartvalues)
        {
            ...
            foreach (var dr_calc in drs_calc)
            {
                ...
                if (dr_prev == null)
                {
                    dr_prev = new P_TAXDEPRECYEAR();
                    dr_prev.CATT = dr_calc.CatID;
                    dr_prev.YR = yr;
                    MyData.DbContextP.P_TAXDEPRECYEAR.Add(dr_prev);
                }
            ...
            MyData.DbContextP.P_TAXDEPRECYEAR.RemoveRange(drs_delete);
        ...
        public void DoAddNewYear()
        {
            ...
            if (table_deprec.Count == 0 &&
                table_catt.Where(d => d.VALUE0 != 0.0M).Count() > 0)
            {
