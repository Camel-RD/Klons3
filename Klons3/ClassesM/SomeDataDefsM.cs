...
public class SomeDataDefsM
{
    ...
    public static string GetAcc21(M_DOCS dr_doc)
    {
        if (IsStoreOurs(dr_doc.XStoreInType))
            return dr_doc.StoreIn.ACC21;
        if (IsStoreOurs(dr_doc.XStoreOutType))
            return dr_doc.StoreOut.ACC21;
        return null;
    }

    public static string GetAcc231(M_DOCS dr_doc)
    {
        if (IsStorePartner(dr_doc.XStoreInType))
            return dr_doc.StoreIn.ACC23;
        if (IsStorePartner(dr_doc.XStoreOutType))
            return dr_doc.StoreOut.ACC23;
        return null;
    }

    public static string GetAcc531(M_DOCS dr_doc)
    {
        if (IsStorePartner(dr_doc.XStoreInType))
            return dr_doc.StoreIn.ACC53;
        if (IsStorePartner(dr_doc.XStoreOutType))
            return dr_doc.StoreOut.ACC53;
        return null;
    }

    public static bool IsCreditDocInTheSameMonth(M_DOCS dr_doc)
    {
        var doctp = dr_doc.XDocType;
        if (!IsCreditDoc(doctp)) return false;
        if (dr_doc.CREDDOCDT == null) return false;
        bool rt = dr_doc.DT.FirstDayOfMonth() == dr_doc.CREDDOCDT.Value.FirstDayOfMonth();
        return rt;
    }
    ...
    public static string GetAccT(M_ROWS dr_row,
        EAccountsForTemplates tacc)
    {
        if (IsInnerDoc(dr_row.Doc.XDocType))
            throw new Exception("Bad call.");
        switch (tacc)
        {
            case EAccountsForTemplates.Nav: return null;
            case EAccountsForTemplates.PVN: return "5731";
            case EAccountsForTemplates.Krājumi: return GetAcc21(dr_row.Doc);
            case EAccountsForTemplates.Debitori: return GetAcc231(dr_row.Doc);
            case EAccountsForTemplates.Kreditori: return GetAcc531(dr_row.Doc);
            case EAccountsForTemplates.Ieņēmumi: return dr_row.Item.Category.ACC6;
            case EAccountsForTemplates.Izmaksas: return dr_row.Item.Category.ACC7;
        }
        return null;
    }
