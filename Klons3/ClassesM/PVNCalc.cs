...
{
    public static DocAccData GetPVNData(M_DOCS dr_doc)
    {
...
public class RowPVNAndAccData
{
    ...
    public string ReadFrom(M_ROWS dr_row, bool readacc)
    {
        ...
        {
            AccBase.DebFin = dr_doc.StoreIn.ACC21;
            AccBase.CredFin = dr_doc.StoreOut.ACC21;
        }
        else
        {
            var table_pvnrates2 = KlonsData.St.DbContextM.BL_M_PVNRATES2;
            var doctp = dr_row.Doc.XDocType;
            IsCreditDocInTheSameMonth = SomeDataDefsM.IsCreditDocInTheSameMonth(dr_doc);
            ...
            if (dr_pvnrate2 == null)
            {
                var pvntypecode = KlonsData.St.DbContextM.M_PVNTYPE.Find(dr_row.Doc.PVNTYPE).CODE;
                var ret = $"Nav izveidots kontējuma shēma priekš:\n";
                ret += $"Likme: {PVNRateCode}\n";
                ret += $"PVN režīms: {pvntypecode}\n";
                ret += $"Darijuma veids: {dr_row.Doc.DocType.CODE}";
                return ret;
            }
            ...
            AccBase.DebFin = SomeDataDefsM.GetAccT(dr_row, dr_pvnrate2.XBaseDebFin);
            AccBase.CredFin = SomeDataDefsM.GetAccT(dr_row, dr_pvnrate2.XBaseCredFin);
            ...
            if (are_we_vat_payer)
            {
                ...
                AccPVN.DebFin = SomeDataDefsM.GetAccT(dr_row, dr_pvnrate2.XPvnDebFin);
                AccPVN.DebPVN = dr_pvnrate2.PVN_DEB_PVN;
                AccPVN.CredFin = SomeDataDefsM.GetAccT(dr_row, dr_pvnrate2.XPvnCredFin);
        ...
        if (!AccBase.IsGood() || !AccPVN.IsGood())
        {
            ...
            ret += $"Artikulu kategorija: {dr_row.Item.Category.CODE}";
            return ret;
        }
    ...
    public static string GetBaseText(M_PVNRATES2 dr_pvnrates2)
    {
        if (dr_pvnrates2.IDPVNTEXT == null) return null;
        int id = dr_pvnrates2.IDPVNTEXT.Value;
        return KlonsData.St.DbContextM.M_PVNTEXTS.Find(id)?.TAG1;
    }

    public static string GetPVNText(M_PVNRATES2 dr_pvnrates2)
    {
        if (dr_pvnrates2.IDPVNTEXT == null) return null;
        int id = dr_pvnrates2.IDPVNTEXT.Value;
        return KlonsData.St.DbContextM.M_PVNTEXTS.Find(id)?.TAG2;
    }
...
public class DocAccData
{
    ...
    public bool ReadRowData(M_DOCS dr_doc, bool readacc)
    {
        Err.Clear();
    ...
    public void GetPVNData(M_DOCS dr_doc)
    {
        PVNBase = 0M;
        PVN = 0M;
        ReversePVN = 0M;
    ...
    public void GetAccData(M_DOCS dr_doc)
    {
        PVNBase = 0M;
        PVN = 0M;
