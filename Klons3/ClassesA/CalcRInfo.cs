...
namespace KlonsA.Classes
{
    public class CalcRInfo
    {
        ...
        public void CalcR(SalarySheetRowInfo sr, DateTime dt1, DateTime dt2)
        {
            ...
            if (dr_likmes == null)
                dr_likmes = DataTasksA.GetRates(dt1.FirstDayOfMonth());
            ...
            for (int i = 0; i < fPersonR.LinkedPeriods.Count; i++)
            {
                ...
                GetIINDeductionsForPerson(wt1, pri.Item1 as A_PERSONS_R, dr_likmes, dt1x, dt2x, CalcVer);
        ...
        public static void GetRatesForPerson(CalcRRow2 wt, A_PERSONS_R drpr, A_RATES drl, DateTime dt)
        {
            ...
            else
            {
                ...
                int iin_rate_type = GetIINRateType(drpr.Person, dt.FirstDayOfMonth());
                if (iin_rate_type == 1)
        ...
        public static void GetIINDeductionsForPerson(CalcRRow2 wt, A_PERSONS_R drpr,
            A_RATES drl, DateTime dt)
        {
            ...
            if (!string.IsNullOrEmpty(drpr.TAXDOC_NO))
            {
                ...
                else
                {
                    ...
                    {
                        wt.ExUntaxedMinimum = GetIINUntaxedMinimum(drpr.Person, dt);
                    }
                }
            }
        ...
        public static void GetIINDeductionsForPerson(CalcRRow2 wt, A_PERSONS_R drpr, 
            A_RATES drl, DateTime dt1, DateTime dt2, int calcver)
        {
            wt.ExUntaxedMinimum = 0.0M;
            wt.ExDependants = 0.0M;
            wt.ExInvalidity = 0.0M;
            ...
            if (!string.IsNullOrEmpty(drpr.TAXDOC_NO))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            wt.ExUntaxedMinimum = GetIINUntaxedMinimum(drpr.Person, dt1);
                        }
                        else
                        {
                            wt.ExUntaxedMinimum = GetIINUntaxedMinimum2(drpr.Person, dt1, dt2);
                        }
                    }
                }
            }
        ...
        public static decimal GetIINUntaxedMinimum(A_PERSONS drpr, DateTime dt)
        {
            var dr_um = drpr.UntaxedMinRows
                .Where(d => d.ONDATE <= dt)
                .OrderBy(d => d.ONDATE)
                .LastOrDefault();
        ...
        public static decimal GetIINUntaxedMinimum2(A_PERSONS drpr, 
            DateTime dt1, DateTime dt2)
        {
            var pi = new PeriodInfo();
            pi.DateFirst = dt1;
            pi.DateLast = dt2;
            var drs = drpr.UntaxedMinRows
                .OrderBy(d => d.ONDATE)
                .ToArray();
            ...
            foreach (var pi2 in pi.LinkedPeriods)
            {
                ...
                decimal rate = (pi2.Item1 as A_UNTAXED_MIN).UNTAXED_MIN;
        ...
        public static int GetIINRateType(A_PERSONS drpr, DateTime dt)
        {
            var dr_um = drpr.UntaxedMinRows
                .Where(d => d.ONDATE <= dt)
                .OrderBy(d => d.ONDATE)
        ...
        public void PrepareList()
        {
            ...
            foreach (var r in ReportRows)
            {
                if (r.Date1 != dt1 && !r.IsTotals)
                {
                    var c1 = new CalcRRow();
                    c1.IsTitle = true;
                    c1.Caption = $"{r.Date1:dd.MM.yyyy}-{r.Date2:dd.MM.yyyy}";
                    list.Add(c1);
                    dt1 = r.Date1;
                }
