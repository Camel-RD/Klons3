...
namespace KlonsA.Classes
{
    ...
    */
    ...
    public class SomeDataDefsA
    {
        ...
        public static List<MyListItemInt> PaylistTpPay { private set; get; } = null;
        ...
        public static bool ParsePlanDayStr(string fs, out EDayPlanId daycode, out float hours)
        {
            ...
            {
                hours = 0.0f;
                daycode = SomeDataDefsA.GetPlanStrId(fs.Trim().ToUpper());
            ...
            if (k > 0)
            {
                ...
                daycode = SomeDataDefsA.GetPlanStrId(daystr);
        ...
        public static bool ParseFactDayStr(string fs, out EDayFactId daycode, out float hours)
        {
            ...
            {
                hours = 0.0f;
                daycode = SomeDataDefsA.GetFactStrId(fs.Trim().ToUpper());
            ...
            {
                var daystr = fs.Substring(0, k).Trim().ToUpper();
                daycode = SomeDataDefsA.GetFactStrId(daystr);
                if (daycode == EDayFactId.Error) return false;
            }
            else
            {
        ...
        public static MyListItemInt[] GetBonusFromItems(EBonusFrom[] bfs)
        {
            ...
            var drs = MyData.DbContextA.BL_A_PLUSMINUS_FROM.OrderBy(d => d.ID).ToArray();
