...
namespace KlonsA.Classes
{
    public class WorkPayCalcInfo
    {
        ...
        public void CalcWorkPay(SalarySheetRowInfo sr, SalaryInfo si = null)
        {
            ...
            PeriodInfo amats_hf;
            if (!sr.Events.Positions.TryGetValue(sr.Row.Position, out amats_hf))
            ...
            for (int i = 0; i < amatirpay.Length; i++)
            {
                ...
                var dr_amati_r = ari_x.Item1 as A_POSITIONS_R;
            ...
            TotalRow._SALARY_PIECEWORK = CalcPieceWorkPay(dt1, dt2, sr.Row.Position);
        ...
        public void CalcPayWithDayRate(SalaryInfo wt, A_POSITIONS_R dr_amati_r)
        {
        ...
        public void CalcPayWithHourRate(SalaryInfo wt, A_POSITIONS_R dr_amati_r)
        {
        ...
        //wt._R_HR not set?
        public void CalcPayWithoutAvPay(SalaryInfo wt, A_POSITIONS_R dr_amati_r)
        {
            wt.PlanedWorkPay = wt.R_HR * (decimal)wt._PLAN_HOURS;
            wt._SALARY_DAY = wt.R_HR * (decimal)wt._FACT_WORK_HOURS;
            wt._SALARY_NIGHT = wt.R_HR_NIGHT * (decimal)wt._FACT_WORK_HOURS_NIGHT;
        ...
        public decimal CalcPieceWorkPay(DateTime dt1, DateTime dt2, A_POSITIONS dr_position)
        {
            var table_gbd = MyData.DbContextA.BL_A_PIECEWORK;
            var d = table_gbd.Where(
                r =>
                r.Position == dr_position &&
                r.DT >= dt1 &&
                r.DT <= dt2
            ).Sum(r => r.PAY);
            return d;
        ...
        {
            var posr = sr.PositionsR.LinkedPeriods[0].Item1 as A_POSITIONS_R;
        ...
        public void PrepareList(string postitle)
        {
            for (int i = 0; i < ReportRows.Count; i++)
            {
                var r = ReportRows[i];
                var jr = JoinRows[i];
                jr.Caption = $"{jr.DateStart:dd.MM.yyyy} - {jr.DateEnd:dd.MM.yyyy}";
            }
