...
namespace KlonsA.Classes
{
    public class TimeSheetRowSetList : List<TimeSheetRowSet>
    {
        ...
        public ErrorList CheckDays(SalaryInfo wt, DateTime dt1, DateTime dt2, A_PERSONS dr_person)
        {
            var error_list = new ErrorList();
            string errstr = $"{dt1.Year}.{dt1.Month}";
            int i, j;
            int i1 = dt1.Day;
            int i2 = dt2.Day;
            ...
            for (i = i1; i <= i2; i++)
            {
                ...
                for (j = 0; j < this.Count; j++)
                {
                    ...
                    {
                        var errs = $"{errstr} nesakrīt darba laika plāna dati dažādiem amatiem dienai: {i}";
                        error_list.AddPersonError(dr_person, errs);
                    }
                }
                iscodeset = false;
                for (j = 0; j < this.Count; j++)
                {
                    ...
                    if (!iscodeset)
                    {
                        ...
                        if (!SomeDataDefsA.CanPlanHaveFact(daycodeplan, daycodefact))
                        {
                            var errs = $"{errstr} nekorekts faktiski nostrādās dienas kods: {i}";
                            error_list.AddPersonError(dr_person, errs);
                        }
                    ...
                    bool b1 = SomeDataDefsA.IsSickDay(daycodefact) ||
                        SomeDataDefsA.IsDayVacation(daycodefact) ||
                        daycodefact == EDayFactId.V;

                    bool b2 = SomeDataDefsA.IsSickDay(daycodefact2) ||
                        SomeDataDefsA.IsDayVacation(daycodefact2) ||
                        daycodefact2 == EDayFactId.V;

                    if (b1 != b2)
                    {
                        var errs = $"{errstr} nesakrīt darba laika dati dažādiem amatiem dienai: {i}";
                        error_list.AddPersonError(dr_person, errs);
                    }
                }
            }
        ...
        public ErrorList CountTotalPlan(IWorkTimeData wt, DateTime dt1, DateTime dt2)
        {
            ...
            for (i = i1; i <= i2; i++)
            {
                ...
                for (j = 0; j < this.Count; j++)
                {
                    ...
                    if (SomeDataDefsA.IsPlanWorkday(daycodeplan) && 
                        (SomeDataDefsA.IsDayForWork(daycodefact) || daycodefact == EDayFactId.B))
                    {
                        if (!planworkdayadded)
                            wt._PLAN_WORK_DAYS++;
        ...
        public ErrorList CountTotalFact(IWorkTimeData wt, DateTime dt1, DateTime dt2)
        {
            ...
            for (i = i1; i <= i2; i++)
            {
                ...
                for (j = 0; j < this.Count; j++)
                {
                    ...
                    bool IsFactWorkDay = SomeDataDefsA.IsDayForWork(daycodefact);
            ...
            out int sickdaysA, out int sickdaysB, out int sickdaysN)
        {
            ...
            for (int i = i1; i <= i2; i++)
            {
                ...
                for (int j = 0; j < this.Count; j++)
                {
                    ...
                    var isFactSickDay = SomeDataDefsA.IsSickDay(daycodefact);
        ...
        public int CountVacationDays(DateTime dt1, DateTime dt2)
        {
            ...
            for (int i = i1; i <= i2; i++)
            {
                ...
                for (int j = 0; j < this.Count; j++)
                {
                    ...
                    var isFactPaidVacation = SomeDataDefsA.IsDayPaidVacation(daycodefact);
