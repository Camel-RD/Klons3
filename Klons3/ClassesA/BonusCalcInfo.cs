...
namespace KlonsA.Classes
{
    ...
    {
        public A_SALARY_PLUSMINUS[] DataRows = null;
        public List<A_SALARY_PLUSMINUS> OrderedDataRows = new List<A_SALARY_PLUSMINUS>();
        ...
        public BonusCalcInfo(CalcRInfo cri, A_SALARY_PLUSMINUS[] rows, bool fillist = false)
        {
            CalcR = cri;
            DataRows = rows;
            MakePfx();
        ...
        public BonusCalcInfo Filter(Func<A_SALARY_PLUSMINUS, bool> f)
        {
            var list = DataRows.Where(d => f(d)).ToArray();
            return new BonusCalcInfo(CalcR, list, PreparingReport);
        }

        public PayFx2 AddRow(SalaryInfo si, A_SALARY_PLUSMINUS dr, 
            decimal dfrom, decimal addamount, int divby)
        {
            ...
            if (PreparingReport)
            {
                ...
                br.Caption = dr.DESCR;
                br.AddTo = SomeDataDefsA.GetEBonusText(dr.XBonusType);
                br.CalcFrom = SomeDataDefsA.GetEBonusFromString(dr.XBonusFrom);
        ...
        private int FindAM(SalaryCalcTInfo scti, A_POSITIONS dr_position)
        {
            for (int i = 0; i < scti.LinkedSCI.Length; i++)
                if (scti.LinkedSCI[i].SR.Row.Position == dr_position) return i;
            return -1;
        }
            ...
            out List<CalcRet> rpfx)
        {
            ...
            for (int i = 0; i < DataRows.Length; i++)
            {
                ...
                {
                    int ind = FindAM(sct, dr.Position);
