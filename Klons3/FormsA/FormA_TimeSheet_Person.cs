...
{
    public partial class FormA_TimeSheet_Person : MyFormBaseF
    {
        public FormA_TimeSheet_Person()
        {
            InitializeComponent();
        ...
        public List<ReportRow> MakeList(A_PERSONS dr_person)
        {
            ...
            var drs_sar_r = dr_person.TimesheetListRows.ToList();
            ...
            foreach (var dr_sar_r in drs_sar_r)
            {
                ...
                {
                    Person = dr_person,
                    Position = dr_sar_r.Position,
                    YR = dr_sar.YR,
                    MT = dr_sar.MT,
                    YRMT = $"{dr_sar.YR}.{dr_sar.MT:00}",
                    Name = name,
                    PositionTitle = s_position
                };
                ...
                if (dr_sar_r.XPlanType == EPlanType.Fixed)
                {
                    ...
                    */
                    ...
                    var drs_darba_laiks_plans = dr_sar_r.TimePlan.TimesheetRows.Where(dr =>
                        dr.TimeSheetListRow == null &&
                        dr.YR == dr_sar.YR &&
                        dr.MT == dr_sar.MT).ToArray();
        ...
        public class TimeSheetItemSet
        {
            ...
            public A_POSITIONS Position = null;
            public string YRMT { get; set; } = null;
            ...
            public void SetRow(A_TIMESHEET row)
            {
                switch (row.XKind1)
                {
            ...
            public ReportRow MakeReportRow(A_TIMESHEET row)
            {
                var ret = new ReportRow();
                ret.Kind = SomeDataDefsA.DarbaLaiksK1[(int)row.KIND1].Val;
                ret.K1 = row.K1.Value;
                for (int i = 0; i < 31; i++)
                {
                    short daycode = row.Dx[i];
            ...
            public List<ReportRow> MakeReportRows()
            {
                ...
                for(int i = 0; i < ret.Count; i++)
                {
                    ...
                    string pos1 = PositionTitle;
                    string pos2 = null;
                    int posbreaklen = 25;
                    if (!PositionTitle.IsNOE() && PositionTitle.Length > posbreaklen)
                    {
                        pos1 = PositionTitle.LeftMax(posbreaklen);
                        pos2 = PositionTitle.Substring(posbreaklen);
                    }
        ...
        public void DoMakeReport()
        {
            if (cbPerson.SelectedIndex == -1 || cbPerson.SelectedValue == null)
                return;
            var dr_person = (A_PERSONS)cbPerson.SelectedValue;
            var reprows = MakeList(dr_person);
