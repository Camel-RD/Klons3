    ...
    public partial class FormA_Persons_DN_lapas : MyFormBaseF
    {
        ...
        public string ApplyChanges()
        {
            ...
            foreach (var change in DataChanges)
            {
                if (change.DNLapaImportType == EDNLapaImportType.AddNew)
                {
                    var dr_new = table_events.NewItem();
                    table_events.Add(dr_new);
                    dr_new.Person = change.PersonRow;
                ...
                {
                    if (change.EventRow == null) continue;
                    change.EventRow.DATE2 = change.EDS_Dt2.Value;
                }
            }
