...
namespace KlonsP.Classes
{
    public class Report_Movement
    {
        ...
        public void MakeSimpleReport()
        {
            ...
            var table_items = MyData.DbContextP.P_ITEMS.Local;
        ...
        public void GetTotalsKeys_Cat1(out List<string> keys, out List<int> ids)
        {
            ...
            ids = new List<int>();
            var drs = MyData.DbContextP.P_CAT1.Local
                .Where(d => d.GROUP == 1)
                .OrderBy(d => d.CODE);
        ...
        public void GetTotalsKeys_Dep(out List<string> keys, out List<int> ids)
        {
            keys = new List<string>();
            ids = new List<int>();
            var drs = MyData.DbContextP.P_DEPARTMENTS.Local
                .Where(d => d.GROUP == 1)
                .OrderBy(d => d.CODE);
            foreach (var dr in drs)
            {
        ...
        public void GetTotalsKeys_Place(out List<string> keys, out List<int> ids)
        {
            keys = new List<string>();
            ids = new List<int>();
            var drs = MyData.DbContextP.P_PLACES.Local
                .Where(d => d.GROUP == 1)
                .OrderBy(d => d.CODE);
            foreach (var dr in drs)
            {
