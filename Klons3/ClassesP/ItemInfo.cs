...
namespace KlonsP.Classes
{
    public class ItemInfo : EventInfo0
    {
        ...
        public void SetFromRow(P_ITEMS dr, bool forrep = false)
        {
            DR = dr;

            Events = new List<EventInfo>();
            var drs_ev = dr.Events
                .OrderBy(d => d.DT)
                .ThenBy(d => d.EVENTRow.ID < 0 ? int.MaxValue / 2 - d.EVENTRow.ID : d.EVENTRow.ID);
        ...
        public void SetEventToInit(P_ITEMS_EVENTS drtoinit)
        {
            ...
            while (i1 < Events.Count || i2 < Events2.Count)
            {
                ...
                else
                {
                    ...
                    if (Events.Count > 0 && Events[0] != curEventInfo)
                    {
                        ...
                        {
                            var editablefields = SomeDataDefsP.GetEditableFields(curEventInfo.XEvent);
