    ...
    public partial class FormM_RepMovementPerItem : MyFormBaseF
    {
        ...
        public string CheckParams()
        {
            ...
            if (tbCatCode.SelectedIndex == -1)
                ...
                IdCat = ((M_ITEMS_CAT)tbCatCode.SelectedValue).ID;
            return "OK";
        }
        ...
        public void MakeReport()
        {
            ...
            List<SP_M_REP_ITEMSAMOUNTS_3> table;
            if (tbStoreCode.SelectedValue == null)
            {
                table = DataLoaderM.GetBy_SP_M_REP_ITEMSAMOUNTS_3(Date1, Date2, IdCat);
            }
            else
            {
                int idstore = (int)tbStoreCode.SelectedValue;
                table = DataLoaderM.GetBy_SP_M_REP_ITEMSAMOUNTS_6(Date1, Date2, IdCat, idstore);
            }
            if (table.Count == 0)
            {
            ...
            foreach (var dr_rep in table)
            {
                if (reprow is null || dr_rep.IDITEM != reprow.Id)
                {
                    ...
                    var dr_item = MyData.DbContextM.M_ITEMS.Find(reprow.Id);
                    reprow.Code = dr_item.BARCODE;
                    reprow.Name = dr_item.NAME;
                    reprow.ItemUnits = dr_item.UnitsRow.CODE;
        ...
        {
            var dr_selected = (M_ITEMS_CAT)tbCatCode.SelectedValue;
            var dr_cat = FormM_ItemsCat.GetItemsCat(dr_selected);
            if (dr_cat == null) return;
            tbCatCode.SelectedValue = dr_cat;
        }

        private void tbStoreCode_ButtonClicked(object sender, EventArgs e)
        {
            var dr_store = (M_STORES)tbStoreCode.SelectedValue;
            dr_store = FormM_Stores.GetStore(dr_store, EStoreType.Partneris);
            if (dr_store == null) return;
            tbStoreCode.SelectedValue = dr_store;
        }
