...
{
    public partial class FormM_Banks : MyFormBaseF
    {
        public FormM_Banks()
        {
            InitializeComponent();
        ...
        public static M_BANKS GetBankId()
        {
            var fm = new FormM_Banks();
            var ret = fm.ShowMyDialogModal();
            if (ret != DialogResult.OK) return null;
            return (M_BANKS)fm.SelectedObject;
        }

        private void SelectCurrent()
        {
            if (dgvRows.CurrentRow == null || dgvRows.CurrentRow.IsNewRow) return;
            var dr = bsRows.GetCurrentItem<M_BANKS>();
