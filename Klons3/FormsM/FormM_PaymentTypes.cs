...
{
    public partial class FormM_PaymentTypes : MyFormBaseF
    {
        public FormM_PaymentTypes()
        {
            InitializeComponent();
            CheckMyFontAndColors();
        }

        private void FormM_PaymentTypes_Load(object sender, EventArgs e)
        {

        }

        public static M_PAYMENTTYPE GetPaymentTypeId(M_STORES dr_store)
        {
            var fm = new FormM_PaymentTypes();
            var ret = fm.ShowMyDialogModal();
            if (ret != DialogResult.OK) return null;
            return (M_PAYMENTTYPE)fm.SelectedObject;
        }

        private void SelectCurrent()
        {
            if (dgvRows.CurrentRow == null || dgvRows.CurrentRow.IsNewRow) return;
            var dr = bsRows.GetCurrentItem<M_PAYMENTTYPE>();
