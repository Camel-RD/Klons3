...
{
    public partial class FormM_Vehicles : MyFormBaseF
    {
        public FormM_Vehicles()
        {
            InitializeComponent();
            CheckMyFontAndColors();
        }

        private void FormM_Vehicles_Load(object sender, EventArgs e)
        {

        }

        public static M_VEHICLES GetVehicle(M_STORES dr_store)
        {
            var fm = new FormM_Vehicles();
            if (!fm.FindStore(dr_store)) return null;
            var ret = fm.ShowMyDialogModal();
            if (ret != DialogResult.OK) return null;
            return (M_VEHICLES)fm.SelectedObject;
        }
        ...
        private void SelectCurrent()
        {
            ...
            var dr = bsRows.GetCurrentItem<M_VEHICLES>();
