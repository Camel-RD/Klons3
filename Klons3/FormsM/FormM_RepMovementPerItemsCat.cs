    ...
    public partial class FormM_RepMovementPerItemsCat : MyFormBaseF
    {
        ...
        public string CheckParams()
        {
            ...
            if (tbCode.SelectedIndex == -1)
                ...
                IdCat = ((M_ITEMS_CAT)tbCode.SelectedValue).ID;
            return "OK";
        }
        ...
        public void MakeReport()
        {
            ...
            var table = DataLoaderM.GetBy_SP_M_REP_ITEMSAMOUNTS_5(Date1, Date2, IdCat);
            if (table.Count == 0)
            {
            ...
            foreach (var dr_rep in table)
            {
                if(reprow is null || dr_rep.IDCAT != reprow.Id)
                {
                    ...
                    var dr_cat = MyData.DbContextM.M_ITEMS_CAT.Find(reprow.Id);
            ...
            var grouplist = MyData.DbContextM.BL_M_ITEMS_CAT
                .Where(x => x.ISGROUP == 1)
                .ToList();
        ...
        {
            var dr_slected = (M_ITEMS_CAT)tbCode.SelectedValue;
            var dr_cat = FormM_ItemsCat.GetItemsCat(dr_slected);
            if (dr_cat == null) return;
            tbCode.SelectedValue = dr_cat;
        }
