...
namespace KlonsM.FormsM
{
    public partial class FormM_Accounts : MyFormBaseF
    {
        ...
        public void FindAcc(string code)
        {
            ...
            {
                var dr = bsAccounts.GetItem<M_ACCOUNTS>(i);
                if (dr.ID == code)
                {
        ...
        private void SelectCurrent()
        {
            ...
            var dr = bsAccounts.GetCurrentItem<M_ACCOUNTS>();
            if (!dgvRows.EndEdit()) return;
            if (!SaveData()) return;
            if (bsAccounts.IsItemDetached(dr)) return;
        ...
        public void DoFilterA()
        {
            ...
            bsAccounts.SetFilter<M_ACCOUNTS>(x => x.TP == FilterAccountType || x.TP == 1);
        }
        ...
        public override bool SaveData()
        {
            ...
            var rt = bsAccounts.SaveDbContext();
            CheckSave();
            return rt.IsSuccess();
        }
        ...
        private bool IsRow0(int rownr)
        {
            ...
            var dr = dgvRows.GetObjectViewItem<M_ACCOUNTS>(rownr);
