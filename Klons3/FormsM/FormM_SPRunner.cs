    ...
    public partial class FormM_SPRunner : MyFormBaseF
    {
        ...
        int GetDBSessionId()
        {
            int ret = MyData.DbContextM.Database
                .SqlQuery<int>($"select current_connection from rdb$database")
                .Single();
            return ret;
        }
        ...
        {
            var sfc = MyData.CurrentConnectionString;
            FbCon = new FbConnection(sfc);
            await FbCon.OpenAsync();
            SessionId = GetDBSessionId();
        ...
        private void ShowException(Exception ex, out bool iscanceled)
        {
            ...
            string msg = DataTasksM.GetErrorInfoFromMessage(ex1.Message);
            if (msg == ex1.Message)
            {
                RunInGUIContext(_ =>
