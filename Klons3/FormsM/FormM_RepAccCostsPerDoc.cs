...
{
    public partial class FormM_RepAccCostsPerDoc : MyFormBaseF
    {
        public FormM_RepAccCostsPerDoc()
        {
            InitializeComponent();
        ...
        public void MakeReport()
        {
            ...
            var table = DataLoaderM.GetBy_SP_M_REP_COSTACC_2(Date1, Date2);
            if (table.Count == 0)
            {
        ...
        private void dgvRows_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            if (e.ColumnIndex == dgcTP.Index)
            {
                ...
                var dr_tp = MyData.DbContextM.M_DOCTYPES.Find(idtp);
                if (dr_tp == null) return;
                e.Value = dr_tp.NAME;
                e.FormattingApplied = true;
                return;
            }
            if (e.ColumnIndex == dgcIdStoreIn.Index ||
                e.ColumnIndex == dgcIdStoreOut.Index)
            {
                int idstore = (int)e.Value;
                var dr_store = MyData.DbContextM.M_STORES.Find(idstore);
                if (dr_store == null) return;
                e.Value = dr_store.CODE;
                e.FormattingApplied = true;
                return;
            }
        }
