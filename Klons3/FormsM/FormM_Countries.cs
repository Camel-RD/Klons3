...
{
    public partial class FormM_Countries : MyFormBaseF
    {
        public FormM_Countries()
        {
            InitializeComponent();
            CheckMyFontAndColors();
        }

        private void FormM_Countries_Load(object sender, EventArgs e)
        {

        }

        public static M_COUNTRIES GetCountryId(M_STORES dr_store)
        {
            var fm = new FormM_Countries();
            var ret = fm.ShowMyDialogModal();
            if (ret != DialogResult.OK) return null;
            return (M_COUNTRIES)fm.SelectedObject;
        }

        private void SelectCurrent()
        {
            if (dgvRows.CurrentRow == null || dgvRows.CurrentRow.IsNewRow) return;
            var dr = bsRows.GetCurrentItem<M_COUNTRIES>();
