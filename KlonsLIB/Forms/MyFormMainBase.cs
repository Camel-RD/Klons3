    ...
    public class MyMainFormBase : MyFormBase
    {
        ...
        public override void SetupMenuRenderer()
        {
            ...
            if (MyWindowListToolStrip != null)
                MyWindowListToolStrip.Renderer = ColorThemeHelper.MyToolStripRenderer;
        }
        ...
        public MyFormBase FindForm<T>()
        {
            foreach (MyFormBase f1 in _myChildren)
            {
                if (f1.GetType() == typeof(T)) return f1;
            }
            return null;
        }
        ...
        protected T ShowFormIfCreated<T>() where T : MyFormBase
        {
            for (int i = _myChildren.Count - 1; i >= 0; i--)
            {
                var f1 = _myChildren[i] as T;
                if (f1 is not null)
                {
                    if (f1.Enabled && !f1.IsMyDialog)
                    {
        ...
        public T ShowForm<T>() where T : MyFormBase, new()
        {
            var f = ShowFormIfCreated<T>();
            if (f != null) return f;
            f = new T();
            _myChildren.Add(f);
            f.MdiParent = this;
        ...
        public T ShowForm<T>(Func<T> func_makeform) where T : MyFormBase
        {
            var f = ShowFormIfCreated<T>();
            if (f != null) return f;
            f = func_makeform();
        ...
        public T ShowFormDialog<T>() where T : MyFormBase, new()
        {
            T f = new T();
            _myChildren.Add(f);
            f.Tag = "modal";
        ...
        public T ShowFormDialog<T>(Func<T> func_makeform) where T : MyFormBase
        {
            T f = func_makeform();
            _myChildren.Add(f);
            f.Tag = "modal";
