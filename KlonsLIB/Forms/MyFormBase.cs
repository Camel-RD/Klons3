...
namespace KlonsLIB.Forms
{
    public partial class MyFormBase : Form
    {
        ...
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public virtual string SelectedValueStr { get; set; }
        ...
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public Action<string> OnSelectedValueStr;
        ...
        protected void SetSelectedValueStr(string value, bool cancel = false)
        {
            ...
            if (Modal || IsMyDialog)
            {
                if (OnSelectedValueStr != null) OnSelectedValueStr(value);
        ...
        protected void SetSelectedValueInt(int value, bool cancel = false)
        {
            if (!this.Validate()) return;

            DialogCanceled = cancel;
            SelectedValueInt = value;
            SelectedValueStr = value.ToString();

            if (Modal || IsMyDialog)
            {
                if (OnSelectedValueInt != null) OnSelectedValueInt(SelectedValueInt);
                if (OnSelectedValueStr != null) OnSelectedValueStr(SelectedValueStr);
                OnSelectedValueCalled = true;
            }
            if (Modal)
            {
        ...
        protected void SetSelectedObject(object value, bool cancel = false)
        {
            if (!this.Validate()) return;
            ...
            if (Modal || IsMyDialog)
            {
                if (OnSelectedObject != null) OnSelectedObject(SelectedObject);
                OnSelectedValueCalled = true;
            }
            if (Modal)
            {
        ...
        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            ...
            try
            {
                ...
                if (Modal || IsMyDialog)
                {
                    if (OnSelectedValueStr != null && !OnSelectedValueCalled) OnSelectedValueStr(null);
