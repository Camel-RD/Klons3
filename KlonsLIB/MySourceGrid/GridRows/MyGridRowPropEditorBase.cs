...
namespace KlonsLIB.MySourceGrid.GridRows
{
    ...
    public class MyGridRowPropEditorBase : MyGridRowBase, IGenericStringDropDownEditorTarget
    {
        ...
        protected void InitControlField(EditorControlBase editor)
        {
            if (editor.Control != null) return;
            var md = Utils.GetMethod(editor.GetType(), "CreateControl");
            if (md == null)
                throw new InvalidOperationException("CreateControl method not found");
            var c = md.Invoke(editor, null) as Control;
            var fd = Utils.GetField(editor.GetType(), "mControl");
            if (fd == null)
                throw new InvalidOperationException("Field mControl not found");
            fd.SetValue(editor, c);
        }
        ...
        public void LinkCell()
        {
            ...
            if (string.IsNullOrEmpty(EditorTemplateName) && ReadOnly)
            {
        ...
        */
        ...
        protected virtual string[] GetValuesA(string propname)
        {
        ...
        string[] IGenericStringDropDownEditorTarget.GetValues(string propname)
        {
            return GetValuesA(propname);
        }
