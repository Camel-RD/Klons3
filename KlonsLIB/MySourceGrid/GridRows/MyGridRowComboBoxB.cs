...
namespace KlonsLIB.MySourceGrid.GridRows
{
    public class MyGridRowComboBoxB : MyGridRowPropEditorBase
    {
        ...
        protected override string[] GetValuesA(string propname)
        {
            if (propname == nameof(ListDisplayMember) || propname == nameof(ListValueMember))
            {
                return MyLib7.Data.DataMemberFieldListProvider.GetDataSourceContents(ListSource).ToArray();
            }
            return base.GetValuesA(propname);
        }
