...
namespace Equin.ApplicationFramework
{
        ...
        ICancelAddNew, ITypedList, IEnumerable<T>, IMyBindingListView
    {
        ...
        public bool HookItemEvents { get; set; } = false;
        ...
        private void ClearObjectViewCache()
        {
            foreach (var ov in _objectViewCache.Values)
            {
                UnHookEditableObjectEvents(ov);
                UnHookPropertyChangedEvent(ov);

            }
            _objectViewCache.Clear();
        }

        public void KillView()
        {
            var empty_source_list = new List<BindingList<T>>();
        ...
        /// <summary>
        /// Occurs when item property is changed
        /// </summary>
        public event MyItemPropertyChangedventHandler ListItemPropertyChanged;
        public static event MyItemPropertyChangedventHandler ListItemPropertyChangedGlobal;
        ...
        private static Func<T, AggregateBindingListView<T>, ObjectView<T>> CreateObjectViewFunc = null;

        public static ObjectView<T> CreateObjectView(T @object, AggregateBindingListView<T> parent)
        {
            if (CreateObjectViewFunc == null)
                CreateObjectViewFunc = CreateInstanceHelper.CreateFunc<ObjectView<T>, T,AggregateBindingListView<T>>();
            return CreateObjectViewFunc(@object, parent);
        }
        ...
        public ObjectView<T> AddNew()
        {
            ...
            ObjectView<T> objectView = CreateObjectView(item, this);
                ...
                for (int i = 0; i < sourceList.Count; i++)
                {
                    ...
                    if (OnFilteringItem(item))
                    {
                        ...
                        else
                        {
                            ...
                            editableObject = CreateObjectView(item, this);
        ...
        ObjectBackupTable _objectBackupTable = new();

        /// <remarks>
        /// Currently unused. Here in case we want to perform actions when
        /// an item edit begins.
        /// </remarks>
        internal virtual void BeginItemEdit(object sender, EventArgs e)
        {
            _objectBackupTable.Backup(sender);
        }
        ...
        /// </remarks>
        internal virtual void CancelledItemEdit(object sender, EventArgs e)
        {
            _objectBackupTable.Restore(sender);
        }
        ...
        internal virtual void EndedItemEdit(object sender, EventArgs e)
        {
            if (sender == null) return;
            if (!_objectBackupTable.HasChanges(sender))
            _objectBackupTable.Remove(sender);
        ...
        private void ItemPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            if (index > -1)
            {
                ...
                // its called on endedit
                OnListChanged(ListChangedType.ItemChanged, index);
            }
            var event_args = new MyItemPropertyChangedEventArgs((T)sender, e.PropertyName);
            ListItemPropertyChanged?.Invoke(this, event_args);
            ListItemPropertyChangedGlobal?.Invoke(this, event_args);
        }
        ...
        protected string _filter_str = null;
        public const string FilterString_Enabled = "Enabled";
        //public const string FilterString_Enabled2 = "Enabled2";
        public const string FilterString_Update = "Update";
        ...
        /// <summary>
        /// Sets the filter currently applied to the view.
        /// </summary>
        /// <param name="filter">must be IItemFilter<T>.</param>
        public void SetFilter(object ofilter)
        {
            ...
            var ifilter = ofilter as IItemFilter<T>;
            if (ifilter == null)
                throw new ArgumentException("Wrong type of argument ofilter.");
            Filter = ifilter;
        }
        ...
        */
        ...
        public class ShouldProvideViewEventArgs : EventArgs
        {
            public string PropertyName = null;
            public bool ShouldProvideView = false;
            public ShouldProvideViewEventArgs(string propertyName)
            {
                PropertyName = propertyName;
            }
        }

        public delegate void ShouldProvideViewEventHandler(object sender, ShouldProvideViewEventArgs e);

        public static event ShouldProvideViewEventHandler ShouldProvideViewGlobal;
        public static string[] DontProvideViewForProperties = null;
        protected internal virtual bool ShouldProvideView(PropertyDescriptor property)
        {
            var rt = ProvidedViewPropertyDescriptor.CanProvideViewOf(property);
            if (!rt) return false;
            if (DontProvideViewForProperties != null &&
                DontProvideViewForProperties.Contains(property.Name))
            {
                return false;
            }
            if (ShouldProvideViewGlobal == null) return true;
            var args = new ShouldProvideViewEventArgs(property.Name);
            ShouldProvideViewGlobal.Invoke(this, args);
            return args.ShouldProvideView;
        }
        ...
        protected internal object CreateProvidedView(ObjectView<T> @object, PropertyDescriptor sourceListProperty)
        {
            ...
            if (list is IMyListViewSource listviewsource)
            {
                list = listviewsource.GetBindingListView();
                return list;
            }
        ...
        internal PropertyDescriptorCollection CachedPropertiesOfObjectView { get; set; }
        internal bool[] ShouldProvideViewOfProperties { get; set; }
    ...
    public class BindingListViewFilteringItemEventArgs : EventArgs
    {
        public object Item { get; private set; }
        public bool Include = true;
        public bool Tested = false;
        public BindingListViewFilteringItemEventArgs(object item)
        {
            Item = item;
        }
    }

    public delegate void BindingListViewFilteringItemEventHandler(object sender, BindingListViewFilteringItemEventArgs e);
