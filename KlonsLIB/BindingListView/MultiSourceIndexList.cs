...
namespace Equin.ApplicationFramework
{
    ...
    {
        private static Func<IList, ObjectView<T>, ListItemPair<T>> CreateListItemPair = null;
        private static Func<ListItemPair<T>, int, KeyValuePair<ListItemPair<T>, int>> CreateKeyValuePair = null;
        
        static MultiSourceIndexList()
        {
            CreateListItemPair = CreateInstanceHelper.CreateFunc<ListItemPair<T>, IList, ObjectView<T>>();
            CreateKeyValuePair = CreateInstanceHelper.CreateFunc<KeyValuePair<ListItemPair<T>, int>, ListItemPair<T>, int>();
        }

        public void Add(IList sourceList, ObjectView<T> item, int index)
        {
            var listitempair = CreateListItemPair(sourceList, item);
            var keyvaluepair = CreateKeyValuePair(listitempair, index);
            Add(keyvaluepair);
