...
namespace Equin.ApplicationFramework
{
    ...
    [Serializable]
    public class ObjectView<T> : INotifyingEditableObject, IDataErrorInfo, INotifyPropertyChanged, 
        ICustomTypeDescriptor, IProvideViews, IMyObjectView
    {
        /// <summary>
        ...
        object IMyObjectView.Object => _object;
        Type IMyObjectView.ObjectType => typeof(T);
        ...
        {
            var props = (this as ICustomTypeDescriptor).GetProperties();
            for (int i = 0; i < props.Count; i++)
            {
                var prop = props[i];
                if (_parent.ShouldProvideViewOfProperties != null &&
                    _parent.ShouldProvideViewOfProperties[i] ||
                    ShouldProvideViewOf(prop))
                {
        ...
        {
            if (_parent.CachedPropertiesOfObjectView != null)
                return _parent.CachedPropertiesOfObjectView;
            ...
            var ret = new PropertyDescriptorCollection(props.ToArray());
            _parent.CachedPropertiesOfObjectView = ret;
            _parent.ShouldProvideViewOfProperties = new bool[ret.Count];
            for (int i = 0; i < ret.Count - 1; i++)
            {
                if (ret[i] is ProvidedViewPropertyDescriptor)
                    _parent.ShouldProvideViewOfProperties[i + 1] = true;
            }
            return ret;
        }
